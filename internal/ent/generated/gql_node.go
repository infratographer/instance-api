...
func (n *Instance) IsNode() {}

// IsNode implements the Node interface check for GQLGen.
func (n *InstanceMetadata) IsNode() {}

// IsNode implements the Node interface check for GQLGen.
func (n *InstanceProvider) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id idx.PrefixedID) (Noder, error) {
	...
	case instance.Table:
		...
		query := c.Instance.Query().
			Where(instance.ID(uid))
		query, err := query.CollectFields(ctx, "Instance")
		if err != nil {
			return nil, err
		}
	...
	case instancemetadata.Table:
		...
		query := c.InstanceMetadata.Query().
			Where(instancemetadata.ID(uid))
		query, err := query.CollectFields(ctx, "InstanceMetadata")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
	...
	case instanceprovider.Table:
		...
		query := c.InstanceProvider.Query().
			Where(instanceprovider.ID(uid))
		query, err := query.CollectFields(ctx, "InstanceProvider")
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []idx.PrefixedID) ([]Noder, error) {
	...
	case instancemetadata.Table:
		query := c.InstanceMetadata.Query().
			Where(instancemetadata.IDIn(ids...))
		query, err := query.CollectFields(ctx, "InstanceMetadata")
		if err != nil {
			return nil, err
		}
	...
	case instanceprovider.Table:
		query := c.InstanceProvider.Query().
			Where(instanceprovider.IDIn(ids...))
		query, err := query.CollectFields(ctx, "InstanceProvider")
		if err != nil {
			return nil, err
		}
