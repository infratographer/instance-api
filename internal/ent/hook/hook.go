...
type InstanceProviderFunc func(context.Context, *ent.InstanceProviderMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f InstanceProviderFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.InstanceProviderMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.InstanceProviderMutation", m)
}
