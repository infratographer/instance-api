...
func (ic *InstanceCreate) SetInstanceProviderID(u uuid.UUID) *InstanceCreate {
	ic.mutation.SetInstanceProviderID(u)
	return ic
}

// SetTenantID sets the "tenant_id" field.
func (ic *InstanceCreate) SetTenantID(u uuid.UUID) *InstanceCreate {
...
func (ic *InstanceCreate) AddMetadatumIDs(ids ...uuid.UUID) *InstanceCreate {
...
func (ic *InstanceCreate) check() error {
	...
	if _, ok := ic.mutation.InstanceProviderID(); !ok {
		return &ValidationError{Name: "instance_provider_id", err: errors.New(`ent: missing required field "Instance.instance_provider_id"`)}
	}
	if _, ok := ic.mutation.TenantID(); !ok {
		return &ValidationError{Name: "tenant_id", err: errors.New(`ent: missing required field "Instance.tenant_id"`)}
	}
	if _, ok := ic.mutation.InstanceProviderID(); !ok {
		return &ValidationError{Name: "instance_provider", err: errors.New(`ent: missing required edge "Instance.instance_provider"`)}
	}
	return nil
}
...
func (ic *InstanceCreate) createSpec() (*Instance, *sqlgraph.CreateSpec) {
	...
	if value, ok := ic.mutation.TenantID(); ok {
		_spec.SetField(instance.FieldTenantID, field.TypeUUID, value)
	...
	if nodes := ic.mutation.InstanceProviderIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			...
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(instanceprovider.FieldID, field.TypeUUID),
			},
		}
		for _, k := range nodes {
